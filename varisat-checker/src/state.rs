...
use super::{
    CheckedProofStep, CheckedSamplingMode, CheckedUserVar, CheckerData, CheckerError,
    ProofProcessor, ProofTranscriptProcessor, ResolutionPropagations,
};
...
/// Return type of [`CheckerState::store_clause`]
...
/// Return type of [`CheckerState::delete_clause`]
#[derive(Copy, Clone, PartialEq, Eq)]
...
impl<'a> Default for CheckerState<'a> {
    fn default() -> CheckerState<'a> {
        CheckerState {
            step: 0,
            next_clause_id: 0,
            literal_buffer: vec![],
...
impl<'a> CheckerState<'a> {
    ...
    pub fn process_unit_conflicts(&mut self) -> Result<(), CheckerError> {
